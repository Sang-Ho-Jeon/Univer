import { View, Text, SafeAreaView, FlatList, KeyboardAvoidingView, TouchableWithoutFeedback } from 'react-native'
...
const MessageScreen = () => {
  ...
  const sendMessage = async (text) => {
    await addDoc(
      collection(db, `matches/${matchInfo.id}/messages`),{
      text,
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-white`}>
        <Header callable title={matchedUser?.clubName}/>
        ...
        >
          <TouchableWithoutFeedback>
            <FlatList
                ...
                renderItem={({item})=>item.uid === user.uid? <SenderMessage {...item}/> : <ReciverMessage {...item} photoURL={matchedUser.photoURL}/>}
            />      
          </TouchableWithoutFeedback>
